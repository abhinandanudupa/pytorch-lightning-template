...
if __name__ == "__main__":
    ...
    exp_name = "exp"
    loss_fn = LossModule(
        losses=[{"mse": {"params": {"reduction": "mean"}, "weight": 1.0}}]
    )
    lmod = LightningWrapper(model=model, loss_fn=loss_fn)
    logger = lightning.pytorch.loggers.TensorBoardLogger(
        f"logs/{exp_name}", name=exp_name
    )
    mc = lightning.pytorch.callbacks.ModelChecpoint(
        monitor="valid/loss",
    ...
    es = lightning.pytorch.callbacks.EarlyStopping(
    ...
    train_ds = torchvision.datasets.datasets.MNIST(
