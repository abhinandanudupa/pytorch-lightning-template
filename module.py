...
class LightningWrapper(lightning.LightningModule):
    def __init__(self, model: torch.nn.Module, loss_fn: torch.nn.Module, lr: float=1e-3, **kwargs):
        super.__init__()
    ...
    def training_step(self, batch, batch_idx):
        x: torch.Tensor, y: torch.Tensor = batch
    ...
    def validation_step(self, batch, batch_idx):
        ...
        out: torch.Tensor = self.model(x)
        loss: Dict[str, torch.Tensor] = self.loss_fn(out, y)
        self.log("valid/loss", loss["total"], prog_bar=True,logger=True, on_epoch=True, sync_dist=True)
